...
const Modules = () => {
  ...
  const handleSavePlan = async (data: Partial<Plan>) => {
    ...
    try {
      ...
      if (editingPlan) {
        ...
        // Criar um novo plano com ID único
        const newPlan: Plan = {
  ...
  const handleDelete = async (id: string | number) => {
    ...
    try {
      ...
      setTimeout(() => {
        // Atualiza a lista de planos (marca como inativo ao invés de remover)
        setPlans(prevPlans => 
  ...
  const handleSelectPlan = (plan: Plan) => {
    setSelectedPlan(prevPlan => {
      // Se o mesmo plano for clicado novamente, desseleciona
      if (prevPlan && prevPlan.id === plan.id) {
        return null;
      }
      // Caso contrário, seleciona o novo plano
      return plan;
    });
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {plans.map((plan) => (
              <ModuleCard
                ...
                isActive={selectedPlan ? selectedPlan.id === plan.id : false}
